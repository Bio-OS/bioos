...
func newWDLClient(options *Options) Client {
	...
	return &wdlImpl{
		endpoint:   options.WDL.Endpoint,
		basePath:   options.WDL.BasePath,
		httpClient: client,
	}
}
...
func (i *wdlImpl) RunWorkflow(ctx context.Context, req *RunWorkflowRequest) (*RunWorkflowResponse, error) {
	...
	resp, err := newReq.Post(fmt.Sprintf("%s%s%s", i.endpoint, i.basePath, runWorkflowPath))
	if err != nil {
		return nil, err
	}
	if resp.IsSuccess() {
...
func runRequest2FormData(req *RunRequest) (map[string]string, error) {
	...
	formData[workflowURL] = req.WorkflowURL
	return formData, nil
}
